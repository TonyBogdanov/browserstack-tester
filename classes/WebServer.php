...
class WebServer extends StreamingServer
{
     ...
     * @return Response
     */
    protected function sendNotFound(): Response
    {
    ...
    /**
     * @param string $path
     * @param callable|null $contentFilter
     * @return Response
     */
    protected function sendFile(string $path, callable $contentFilter = null): Response
    {
        if (!is_file($path)) {
        ...
        $mime = isset($this->mimeTypes[$ext]) ? $this->mimeTypes[$ext] : 'text/plain';
        $content = file_get_contents($path);

        if (is_callable($contentFilter)) {
            $content = call_user_func($contentFilter, $content);
        }
     ...
     * @return Response
     */
    protected function sendContent(string $mime, string $content): Response
    {
        return new Response(200, [
            'Content-Type' => $mime,
    ...
    public function __invoke(ServerRequestInterface $request): Response
    {
        ...
        return $this->sendFile($this->root . $path);
    }

    /**
     * @param callable|null $stopped
     * @return $this
     */
    public function start(callable $stopped = null)
    {
        $loop = Factory::create();
        ...
        $loop->addPeriodicTimer(0.01, function () use ($loop, $stopped) {
            if (0 < count($this->result)) {
                ...
                if (is_callable($stopped)) {
                    call_user_func($stopped, $result);
                }
            }
        });
        $loop->run();
